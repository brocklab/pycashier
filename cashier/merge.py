...
def merge_single(sample, fastqs, sourcedir, threads, **kwargs):
    ...
    if not os.path.isfile(merged_barcode_fastq):
        ...
        files = [os.path.splitext(f)[0]
                 for f in files]  # replace with sample dict of files
        ...
        command = 'pear -f {} -r {} -o {} -j {} {}'.format(
            files[0], files[1], merged_barcode_file, threads, pear_args)
        args = shlex.split(command)
        p = subprocess.run(args)
        ...
        os.rename(merged_barcode_file + '.assembled.fastq',
                  merged_barcode_fastq)
    else:
