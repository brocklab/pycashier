...
def read_filter(sample,filter_count,filter_percent,quality,ratio,distance,**kwargs):
    ...
    if filter_count:
        print('Removing sequences with less than {} total occurences'.format(filter_count))
        filter_by_count(file_in,filter_count)
    
    else:
        print('Removing sequences with less than {}% of the total reads per sample'.format(filter_percent))
